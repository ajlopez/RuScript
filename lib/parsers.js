...
function CallExpression(name, expr) {
    this.evaluate = function (context) {
        var fun = context.getLocalValue(name);
        return fun(expr.evaluate(context));
    }
}
...
var rules = [
    ...
    // Call
    get('Name', 'Expression').generate('Expression', function (values) { return new CallExpression(values[0].getName(), values[1]); }),
    get('Name', 'LeftParenthesis', 'Expression', 'RightParenthesis').generate('Expression', function (values) { return new CallExpression(values[0].getName(), values[2]); }),
