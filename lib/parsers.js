...
function CallExpression(name, exprs) {
    this.evaluate = function (context) {
        var fun = context.getLocalValue(name);
        var args = [];
...
var rules = [
    ...
    get('StatementList', 'Statement').generate('StatementList', function (values) { var list = values[0]; list.push(values[1]); return list; }),
    get('Statement').generate('StatementList', function (value) { return [value]; }),
    
    // Class
    
    get('class', 'Name', 'EndOfStatement', 'Suite', 'end').generate('Expression', function (values) { return new ClassExpression(values[1].getName(), values[3]); }),
    ...
    // Call
    get('Name', 'ExpressionList').generate('Expression', function (values) { return new CallExpression(values[0].getName(), values[1]); }),
    get('Name', 'LeftParenthesis', 'ExpressionList', 'RightParenthesis').generate('Expression', function (values) { return new CallExpression(values[0].getName(), values[2]); }),
    get('Name', 'LeftParenthesis', 'RightParenthesis').generate('Expression', function (values) { return new CallExpression(values[0].getName(), []); }),
